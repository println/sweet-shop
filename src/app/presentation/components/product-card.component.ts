    ...
    <div class="group/card flex flex-col h-full">
      ...
      <div class="relative aspect-square overflow-hidden bg-base-200 mb-3 cursor-pointer rounded-[4px]" [routerLink]="[routes.products.path, product.id]">
        ...
        <div class="absolute top-2 right-2 flex flex-col gap-1 items-end">
          <span *ngIf="product.isNew" class="badge badge-sm bg-base-100 text-base-content border-none font-bold shadow-sm">NEW</span>
          <span *ngIf="product.originalPrice && product.originalPrice > product.price" class="badge badge-sm bg-primary text-primary-content border-none font-bold shadow-sm">
      ...
      <div class="flex flex-col gap-1 flex-1">
        ...
        <div class="flex items-center gap-1 text-xs text-base-content/60 mb-1">
          <div class="flex text-yellow-500">
            <span *ngFor="let star of [1,2,3,4,5]">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" [class.fill-current]="star <= (product.rating || 5)" [class.text-base-300]="star > (product.rating || 5)" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="flex gap-2 mt-2">
          <!-- Color circles mock -->
          <div class="w-4 h-4 rounded-full bg-[#5D4037] border border-base-300 cursor-pointer hover:scale-110 transition-transform" title="Chocolate"></div>
          <div class="w-4 h-4 rounded-full bg-[#F5F5DC] border border-base-300 cursor-pointer hover:scale-110 transition-transform" title="Baunilha"></div>
        </div>
      </div>
    </div>
  `
